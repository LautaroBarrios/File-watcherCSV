...
> Al no tener una URL de API va a arrojar un error hasta que se ingrese una correcta o modifique la función `sendFileToAPI()` que se encuentra a partir de la línea 151 para que haga lo que usted desea con los datos. **También, puede descomentar la línea 172 para ver qué datos se enviarían sin la necesidad de colocar una URL de API aún(ten en cuenta que no se borrarán los archivos de la carpeta en caso de utilizar esa línea).**
